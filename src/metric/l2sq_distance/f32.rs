#[cfg(use_scalar)]
fn l2sq_distance_f32_scalar(a: &[f32], b: &[f32]) -> f32 {
    let mut sum = 0.0;
    for i in 0..a.len() {
        let diff = a[i] - b[i];
        sum += diff * diff;
...
unsafe fn l2sq_distance_f32_avx2(a: &[f32], b: &[f32]) -> f32 {
    #[cfg(target_arch = "x86")]
    use core::arch::x86::*;
...
unsafe fn l2sq_distance_f32_sse2(a: &[f32], b: &[f32]) -> f32 {
    #[cfg(target_arch = "x86")]
    use core::arch::x86::*;
    #[cfg(target_arch = "x86_64")]
    use core::arch::x86_64::*;
...
unsafe fn l2sq_distance_f32_neon(a: &[f32], b: &[f32]) -> f32 {
...
pub fn l2sq_distance_f32(a: &[f32], b: &[f32]) -> f32 {
    debug_assert_eq!(a.len(), b.len());

    #[cfg(use_avx2)]
    {
        unsafe { l2sq_distance_f32_avx2(a, b) }
    }

    #[cfg(use_sse2)]
    {
        unsafe { l2sq_distance_f32_sse2(a, b) }
    }

    #[cfg(use_neon)]
    {
        unsafe { l2sq_distance_f32_neon(a, b) }
    }

    #[cfg(use_scalar)]
    {
        l2sq_distance_f32_scalar(a, b)
    }
}
