#[cfg(use_scalar)]
fn l2sq_distance_i8_scalar(a: &[i8], b: &[i8]) -> f32 {
    let mut sum = 0;
    for i in 0..a.len() {
...
unsafe fn l2sq_distance_i8_avx2(a: &[i8], b: &[i8]) -> f32 {
    #[cfg(target_arch = "x86")]
    use core::arch::x86::*;
    ...
    if len >= 32 {
        ...
        while i < end {
            ...
            // Accumulate
            let sum_chunk = _mm256_add_epi32(sq_low, sq_high);
...
unsafe fn l2sq_distance_i8_sse2(a: &[i8], b: &[i8]) -> f32 {
    #[cfg(target_arch = "x86")]
    use core::arch::x86::*;
    #[cfg(target_arch = "x86_64")]
    use core::arch::x86_64::*;
    ...
    if len >= 16 {
        ...
        while i < end {
            ...
            // Accumulate
            let sum_chunk = _mm_add_epi32(sq_low, sq_high);
...
unsafe fn l2sq_distance_i8_neon(a: &[i8], b: &[i8]) -> f32 {
    ...
    if len >= 16 {
        ...
        while i < end {
            ...
            // Compute difference
            let diff_low = vsubq_s16(a_low, b_low);
            let diff_high = vsubq_s16(a_high, b_high);
...
pub fn l2sq_distance_i8(a: &[i8], b: &[i8]) -> f32 {
    debug_assert_eq!(a.len(), b.len());

    #[cfg(use_avx2)]
    {
        unsafe { l2sq_distance_i8_avx2(a, b) }
    }

    #[cfg(use_sse2)]
    {
        unsafe { l2sq_distance_i8_sse2(a, b) }
    }

    #[cfg(use_neon)]
    {
        unsafe { l2sq_distance_i8_neon(a, b) }
    }

    #[cfg(use_scalar)]
    {
        l2sq_distance_i8_scalar(a, b)
    }
}
